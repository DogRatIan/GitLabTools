# Python 2.7
#  Clone all projects from a old GitLab server (V3 API)
import os
import sys
import shutil
import json
...
gConfig = {"apiToken":"", "server":"", "httpUsername":"", "httpPassword":"", "actualRun":False, "outputDir":"clone_v3"}
...
def ShowHelp():
    ...
    print ("    -p PASSWORD, --password=PASSWORD    Set HTTPS password")
    print ("    -a, --actualRun                     Without this, the clone action will skip.")
    print ("    -h, --help                          Show this help.")
    print (" ")
    print ("Example: python %s -t 123456 -h git.host.com:8080 -u admin -p 1234 -a" % sys.argv[0])

def GetArguments():
    try:
        opts, args = getopt.getopt(sys.argv[1:],"aht:s:u:p:",["token=","server=","user=","password="])
...
def GetProjectListHeader(aItem):
    try:
        url = "https://" + gConfig["server"] + "/api/v3/projects"
...
def GetProjectPage(aPage):
    try:
        url = "https://" + gConfig["server"] + "/api/v3/projects" + "?page=" + str(aPage)
        print ("URL: %s" % url)
        ctx = ssl._create_unverified_context()
        req = urllib2.Request(url)
        req.add_header ("PRIVATE-TOKEN", gConfig["apiToken"])
...
def SaveProjectList (aTargetFile, j_project_list):
    try:
        ...
        print "SaveProjectList: I/O error({0}): {1}".format(e.errno, e.strerror)
    except:
        print("SaveProjectList: Unexpected error:", sys.exc_info()[0])
    return False
...
try:
    ...
    print("Prepare output directory: %s" % error) 
    sys.exit (1)
...
for x in j_project_list:
    ...
    try: 
        ...
        cmd += " && env GIT_SSL_NO_VERIFY=true git clone --mirror " + http_url.replace ("https://", "https://" + gConfig["httpUsername"] + ":" + gConfig["httpPassword"] + "@")
        print (cmd)
